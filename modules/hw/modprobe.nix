...
let
  ...
  inherit (lib)
    mkIf mkAttrs mkMerge mapAttrs' nameValuePair flip concatMapStrings
    filterAttrs mapAttrs toInt mapAttrsToList concatLists attrValues;
  inherit (lon-lib)
    mkDisableOption mkEnableFrom mkParanoiaFrom fileNamesNoExt mkEnableDef
    mkPersistDirs;
...
in
{
  options.lonsdaleite.hw.modules =
    ...
    // (mkParanoiaFrom [ "hw" ] [ "" "" "" ]) // (mapAttrs'
      (n: v: nameValuePair n (mkDisableOption "Disables ${n} modules"))
      modules)
    // { };
  ...
  config = mkIf cfg.enable {
    ...
    boot = {
      blacklistedKernelModules = concatLists (mapAttrsToList
        (n: v:
          concatLists
            (attrValues (filterAttrs (n': v': cfg.paranoia >= toInt n') v)))
