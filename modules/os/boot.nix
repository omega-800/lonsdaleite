...
let
  cfg = config.lonsdaleite.os.boot;
  inherit (lib) mkForce optionals mkIf;
  inherit (lon-lib) mkEnableFrom;
in
{
  options.lonsdaleite.os.boot = mkEnableFrom [ "os" ] "Hardens boot process / sets hardened flags";
      ...
      kernelParams =
        [
          ...
          */
          ...
          # TODO: set either of these to true depending on cpu (doesn't matter if both are set, actually)
